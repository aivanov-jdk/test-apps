/*
 cl -DUNICODE rbma.cpp gdi32.lib user32.lib msimg32
*/
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow) {
 ...
 HBITMAP MakeBlank()
{
    int w = GetSystemMetrics(SM_CXMENUCHECK); // default glyph width  🡒 cite turn0search0
    int h = GetSystemMetrics(SM_CYMENUCHECK); // default glyph height 🡒 cite turn0search0
    BITMAPINFO bmi = {};
    bmi.bmiHeader.biSize        = sizeof(BITMAPINFOHEADER);
    bmi.bmiHeader.biWidth       =  w;
    bmi.bmiHeader.biHeight      = -h;          // top-down DIB
    bmi.bmiHeader.biPlanes      = 1;
    bmi.bmiHeader.biBitCount    = 32;          // BGRA 8-8-8-8
    bmi.bmiHeader.biCompression = BI_RGB;
    void* pvBits = nullptr;                   // memory zero-initialised → alpha = 0
    return CreateDIBSection(nullptr, &bmi, DIB_RGB_COLORS, &pvBits, nullptr, 0);
}

HBITMAP MakeRadio(HWND hwnd, BOOL on)
{
    int w = GetSystemMetrics(SM_CXMENUCHECK); // size Windows expects
    int h = GetSystemMetrics(SM_CYMENUCHECK);

    HDC hdc = GetDC(NULL);
    HBITMAP bmp = CreateCompatibleBitmap(hdc, w, h);   // empty bitmap
    HDC mem  = CreateCompatibleDC(hdc);
    HGDIOBJ oldObj = SelectObject(mem, bmp);

    RECT r = { 0, 0, w, h };
    UINT uType = DFC_MENU;
    UINT state = DFCS_MENUBULLET;
    if (on) state |= DFCS_CHECKED;
    DrawFrameControl(mem, &r, uType, state);      // draw the glyph
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        HBITMAP hBitmapTransparent = MakeBlank();
        HBITMAP hBitmapRadio = MakeRadio(hwnd, TRUE);
...
}
{
        ...
        mii4.hbmpUnchecked = hBitmapTransparent;
        mii4.hbmpChecked = hBitmapRadio;

        mii4.fMask = MIIM_STRING | MIIM_BITMAP | MIIM_ID | MIIM_CHECKMARKS;
